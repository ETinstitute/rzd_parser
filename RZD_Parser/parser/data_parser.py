...
class DataParser:
    ...
    def parse_values(self, page_text: str) -> Row:
        ...
        result = result | {f'{match[0]} млн тонн': self.__extract_value(match[1]) for match in matches}

        matches = re.findall(self.general_pattern_tis, page_text)
        result = result | {f'{match[0]} млн тонн': self.__extract_value(match[1]) / 1000 for match in matches}
